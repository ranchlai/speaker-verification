...
class ResNetSE34V2(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(
            1, num_filters[0], kernel_size=3, stride=1, padding=1)
        ...
        self.attention = nn.Sequential(
            nn.Conv1d(
                num_filters[3] * outmap_size, 128, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(128),
            nn.Conv1d(
                128, num_filters[3] * outmap_size, kernel_size=1),
            nn.Softmax(dim=2), )
    ...
    def forward(self, x):
        ...
        if self.encoder_type == "SAP":
            ...
            sg = torch.sqrt((torch.sum((x**2) * w, dim=2) - mu**2).clamp(
                min=1e-5))
            x = torch.cat((mu, sg), 1)
